...
var lose_scene = preload("res://Quiz Mode/popup scene/lose_popup4.tscn")
var win_scene = preload("res://Quiz Mode/popup scene/win_popup4.tscn")
var fireball_scene = preload("res://Quiz Mode/Lesson4 Quiz/fireball4.tscn")
var virusfireball_scene = preload("res://Quiz Mode/Lesson4 Quiz/virusfireball4.tscn")
...
var questions := [
{"text": "What is the main purpose of an if statement?", "answers": ["Repeat code", "Store data", "Make decisions", "Stop program"], "correct": 2},
	{"text": "Which symbol is used to compare equality?", "answers": ["==", "=", "=>", "<="], "correct": 0},
	{"text": "What keyword follows if to handle another condition?", "answers": ["next", "elseif", "else if", "switch"], "correct": 2},
	{"text": "What executes if all conditions are false?", "answers": ["if", "switch", "else", "while"], "correct": 2},
	{"text": "Which control structure is best for multiple fixed options?", "answers": ["loop", "switch", "if", "method"], "correct": 1},
	{"text": "What must each switch case end with?", "answers": ["break", "exit", "stop", "done"], "correct": 0},
	{"text": "What data types can switch work with?", "answers": ["int, char, String", "boolean only", "float only", "All types"], "correct": 0},
	{"text": "What happens if break is missing in switch?", "answers": ["Error", "Next case runs", "Stops early", "Skips all"], "correct": 1},
	{"text": "What is a nested if statement?", "answers": ["if in a loop", "if inside another if", "if after switch", "if with return"], "correct": 1},
	{"text": "Which statement checks multiple conditions quickly?", "answers": ["for", "switch", "else", "do while"], "correct": 1},
	{"text": "What is the purpose of the default case?", "answers": ["Stop program", "Run if no match", "Repeat cases", "Restart code"], "correct": 1},
	{"text": "What is the symbol for NOT?", "answers": ["!", "&", "|", "?"], "correct": 0},
	{"text": "Which condition means both must be true?", "answers": ["&&", "||", "!", "&"], "correct": 0},
	{"text": "Which condition means one must be true?", "answers": ["||", "&&", "&", "!"], "correct": 0},
	{"text": "What is true about an if-else chain?", "answers": ["Only one block runs", "All blocks run", "None run", "Random run"], "correct": 0}
]
...
func correct():
	...
	player_sprite.play("ATTACK4")
	await get_tree().create_timer(0.3).timeout
	shoot_fireball()
	await get_tree().create_timer(1.5).timeout
	if not quiz_finished:
		player_sprite.play("IDLE4")

func wrong():
	if wrong_sound: wrong_sound.play()
	monster_sprite.play("ATTACKm4")
	await get_tree().create_timer(0.25).timeout
	shoot_virus()
	await get_tree().create_timer(1.5).timeout
	if not quiz_finished:
		monster_sprite.play("IDLEm4")
...
func hit_monster():
	...
	monster_sprite.play("HITm4")
	flash(monster_sprite)
	await get_tree().create_timer(0.5).timeout
	
	if GameState.virus_health <= 0:
		monster_sprite.play("DEADm4")
