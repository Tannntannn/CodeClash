...
func display_progress():
	...
	print("🔍 Debug: DragDropState found:", drag_drop_state != null)
	print("🔍 Debug: ScoreLabel found:", score_label != null)
	...
	if drag_drop_state and score_label:
		...
		if not drag_drop_state:
			print("❌ DragDropState is null")
		if not score_label:
...
func _on_button_2_pressed() -> void:  # Fixed: _on_ not *on*
	...
	if not is_inside_tree():
		return
	var err = get_tree().change_scene_to_file("res://lesson 2/Lesson2Start.tscn")
	if err != OK:
