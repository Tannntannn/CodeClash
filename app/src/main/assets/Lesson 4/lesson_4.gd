...
@onready var score_label = $Panel2/ScoreLabel  # Add a label in your UI for score
...
func _ready():
	...
	if drag_drop_state:
		...
		drag_drop_state.set_lesson("Lesson 4")
	
	# Initialize blocks for Lesson 4
	correctly_placed_blocks["ScannerscbuttonL4"] = false
	correctly_placed_blocks["LetterCbuttonL4"] = false
	correctly_placed_blocks["intgradebuttonL4"] = false
	correctly_placed_blocks[">=buttonL4"] = false
	correctly_placed_blocks["elseifbuttonL4"] = false
...
func start_countdown():
	game_timer.start()
	print("Lesson 4 started! Timer counting down...")
...
func _on_hint_button_pressed():
	...
	var block_to_zone = {
		"ScannerscbuttonL4": {"zone": "ScannerscL4", "text": "Scanner sc"},
		"LetterCbuttonL4": {"zone": "LetterGradeCL4", "text": "Letter Grade: C"},
		"intgradebuttonL4": {"zone": "intgradeL4", "text": "int grade"},
		">=buttonL4": {"zone": ">=L4", "text": ">="},
		"elseifbuttonL4": {"zone": "elseifL4", "text": "else if"}
	}
...
func show_button_error(button_name: String):
	var button = null
	var possible_paths = [button_name, "Panel/" + button_name, "Panel2/" + button_name, "$" + button_name]
	for path in possible_paths:
		if has_node(path):
...
func reset_all_button_colors():
	reset_button_color("ScannerscbuttonL4")
	reset_button_color("LetterCbuttonL4") 
	reset_button_color("intgradebuttonL4")
	reset_button_color(">=buttonL4")
	reset_button_color("elseifbuttonL4")
...
func puzzle_completed():
	...
	var success_scene = preload("res://Lesson 4/ProlemSolvedL4.tscn")
...
func mistakes_limit_reached():
	...
	var retry_scene = preload("res://Lesson 4/RetryLesson4.tscn")
...
func submit_score_to_android():
	print("🎯 Lesson4: Submitting score to Android - Score:", current_score)
	
	# Get the AppPlugin instance
	var app_plugin = Engine.get_singleton("AppPlugin")
	if app_plugin:
		print("🎯 Lesson4: Calling AppPlugin.submitScore()")
		app_plugin.submitScore(current_score, 1)  # 1 attempt for drag and drop
	else:
		print("❌ Lesson4: AppPlugin not found!")
